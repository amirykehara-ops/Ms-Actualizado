...
def create_order(event, context):
    try:
        ...
        # Publicamos evento en EventBridge (Step Function se dispara por regla)
        event_detail = {
            "orderId": order_id,
            "input": {
                "customerId": body["customerId"],
...
def get_orders_by_customer(event, context):
    try:
        ...
        return {"statusCode": 200, "body": json.dumps({"orders": orders}, default=decimal_default)}
...
def create_customer(event, context):
    try:
        ...
        return {"statusCode": 201, "body": json.dumps({"message": "Customer created successfully", "customerId": customer_id})}
