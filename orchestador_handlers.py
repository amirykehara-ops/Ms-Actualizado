...
EVENT_BUS_NAME = os.environ.get("EVENT_BUS_NAME", "pf-pardos-bus")
...
def task_cooking(event, context):
    ...
    _put_step_record(order_id, "COOKING", "IN_PROGRESS", started_at)
    _update_order_current_step(order_id, "COOKING", status="IN_PROGRESS")

    finished_at = datetime.utcnow().isoformat()
    _put_step_record(order_id, "COOKING", "DONE", started_at, finished_at)
...
def task_packaging(event, context):
    ...
    started_at = datetime.utcnow().isoformat()
    _put_step_record(order_id, "PACKAGING", "IN_PROGRESS", started_at)
    _update_order_current_step(order_id, "PACKAGING", status="IN_PROGRESS")

    finished_at = datetime.utcnow().isoformat()
    _put_step_record(order_id, "PACKAGING", "DONE", started_at, finished_at)
...
def task_delivery(event, context):
    ...
    started_at = datetime.utcnow().isoformat()
    _put_step_record(order_id, "DELIVERY", "IN_PROGRESS", started_at)
    _update_order_current_step(order_id, "DELIVERY", status="IN_PROGRESS")

    finished_at = datetime.utcnow().isoformat()
    _put_step_record(order_id, "DELIVERY", "DONE", started_at, finished_at)

    # Marcamos la orden como DELIVERED
    pk = f"TENANT#pardos#ORDER#{order_id}"
    orders_table.update_item(
        Key={"PK": pk, "SK": "METADATA"},
